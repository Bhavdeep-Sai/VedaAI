...
export function AssignmentCard({ assignment }: AssignmentCardProps) {
  ...
  return (
    <div
      className={`card p-5 cursor-pointer hover:shadow-md transition-all duration-200 animate-fade-in group ${
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3 mb-3">
        <h3 className="font-semibold text-[var(--text-primary)] text-sm leading-snug flex-1 min-w-0 group-hover:text-brand transition-colors">
          {assignment.title}
        </h3>

        <DropdownMenu>
          <DropdownMenuTrigger
            className="flex-shrink-0 w-7 h-7 flex items-center justify-center rounded-lg hover:bg-[var(--bg-main)] text-[var(--text-muted)] hover:text-[var(--text-primary)] transition-colors opacity-0 group-hover:opacity-100"
